...
| `Model not found` / `lms load` fails | Re-run `./deploy_local.sh`. Playbooks run `lms get --mlx --yes` (macOS), poll disk with `lms ls` fallbacks (`--variants`, `--llm`, plain `--json`), fuzzy-match keys like `gemma-4-12b-it-mlx` against `vars.yml` repo IDs, and accept `Model already downloaded` hints from `~/.hermes/logs/lms-get.log`. Logs: `lms-get.log`, `lms-load.log`. If MLX runtime missing: LM Studio → Settings → Runtime → update MLX |
