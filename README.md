...
| `Model not found` / `lms load` fails | Re-run `./deploy_local.sh`. Playbooks run `lms get --gguf --yes` for `lmstudio_model_download_url`, verify the vars.yml model is on disk, then `lms load` **only** `lmstudio_model` from vars.yml. Logs: `lms-get.log`, `lms-load.log` |
...
| `Model not found` on manual `lms load` | LM Studio model keys are case-sensitive. Run `lms ls` for the exact key from vars.yml (e.g. `google/gemma-4-12b@q4_k_m`, not `@Q4_K_M`). Deploy runs `lms load` for that key only. |
