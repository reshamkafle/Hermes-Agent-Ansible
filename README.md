...
| `Model not found` / `lms load` fails | Re-run `./deploy_local.sh`. Playbooks always run `lms get --mlx --yes` (macOS), poll `lms ls --variants` until the model appears, then try every matching key from disk. Logs: `~/.hermes/logs/lms-get.log`, `lms-load.log`. If MLX runtime missing: LM Studio → Settings → Runtime → update MLX |
